...
export const useHostAppStore = defineStore("hostAppStore", () => {
  ...
  const projectModelGroups = computed(() => {
    ...
    for (const model of documentModelStore.value.models) {
      let project = projectModelGroups.find(
  ...
  const patchModel = async (
    ...
    await app.$baseBinding.updateModel(
      documentModelStore.value.models[modelIndex]
    );
  };
  ...
  app.$topLevelExceptionHandlerBinding?.on(
  ...
  const getHostAppVersion = async () =>
    (hostAppVersion.value =
      await app.$baseBinding.getSourceApplicationVersion());
  ...
  const refreshSendFilters = async () => {
    ...
    if (navisworksSavedSetsFromSendFilters) {
      navisworksAvailableSavedSets.value =
        navisworksSavedSetsFromSendFilters.items;
    }
  ...
  const tryToUpgradeSelectSendFilters = async () => {
    for (const model of documentModelStore.value.models) {
      ...
      const newSelectFilter =
        availableSelectSendFilters.value[existingSelectFilter.id];
      if (!newSelectFilter) {
        continue;
      }
      ...
      const updatedSelectedItems = existingSelectFilter.selectedItems.map(
        (selected) => {
          const upgraded = newSelectFilter.items.find(
    ...
    modelCards.forEach(async (modelCard) => {
      ...
      settingIds?.forEach((id) => {
        ...
        if (!existingSetting) {
          ...
          const currentEnum = sendSettings.value?.find(
            (s) => s.id === id
          )?.enum;
          if (
            currentEnum &&
            existingSetting.enum.length !== currentEnum.length
          ) {
            idsToUpgrade.push(id);
          }
        }
  ...
  app.$baseBinding.on(
    ...
    () =>
      setTimeout(async () => {
        ...
        void tryToUpgradeModelCardSettings(
          sendSettings.value || [],
          "SenderModelCard"
        );
